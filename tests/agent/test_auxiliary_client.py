...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_honors_base_url_override(self):
        class _Entry:
            access_token = "pooled-codex-token"
            base_url = "https://chatgpt.com/backend-api/codex"
        ...
        with (
            ...
            client, model = _build_codex_client(
                "codex-test-model",
                base_url_override="http://127.0.0.1:2455/backend-api/codex/",
            )
    ...
    def test_codex_loopback_override_works_without_pool_entry(self):
        class _Pool:
            def has_credentials(self):
                return True
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("agent.auxiliary_client._read_codex_access_token", side_effect=AssertionError("local proxy should not need Codex OAuth")),
        ):
            from agent.auxiliary_client import _build_codex_client
